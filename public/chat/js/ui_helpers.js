...
// function buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId, messageSenderId);
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, msid, status){
  var messageAttach;
  ...
  if (msid != myId) {
    var messageHtml =
      '<div class="list-group-item notme" style="text-align:left;" id="'+messageId+'" onclick="clickToDeleteMsg('+"'"+messageId+"'"+')">'+
        '<time datetime="'+messageDateSent+ '" class="pull-right">'
         +jQuery.timeago(messageDateSent)+
        '</time>'+

        '<h4 class="list-group-item-heading id='+msid+'">'+messageSenderId+'</h4>'+
  ...
  if(msid == myId) {
    var messageHtml =
      '<div class="list-group-item me" style="text-align:right;" id="'+messageId+'" onclick="clickToDeleteMsg('+"'"+messageId+"'"+')">'+
        // '<time datetime="'+messageDateSent+ '" class="pull-right">'
        //  +jQuery.timeago(messageDateSent)+
        // '</time>'+

        '<h4 class="list-group-item-heading id='+msid+'">You</h4>'+
        '<p class="list-group-item-text">'+
          messageTextHtml +
        '</p>'
        +delivered+read+
...
function buildUserHtml(userLogin, userId, isNew) {
  ...
  if(userId == QBoccupantsId){
    userHtml += " class='col-md-12 col-sm-12 col-xs-12 users_form active' onclick='";
  }
  else {
