    ...
    if (messages) {
      ...
      if(messages.items.length === 0) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId, messageSenderId);
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  var messageHtml = buildMessageHTML(msg.body, userLogin, new Date(), attachmentFileId, msg.id, userId);
...
function clickToDeleteMsg(mId) {
  // var messageId = mId;
  //   $('div#'+messageId).click(function() {
  //     var r = confirm("Deleted message cannot be retrived");
  //     if (r == true) {
  //       var params = {};
  //       QB.chat.message.delete(messageId, params, function(err, res) {
  //         if (res) {
  //           alert("message deleted");
  //           triggerDialog('578c9669a0eb472091000005')
  //         }else{
  //           console.log(err);
  //         }
  //       });          
